...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            image[i][j].rgbtRed = originalRed[i][j];
            image[i][j].rgbtGreen = originalGreen[i][j];
            image[i][j].rgbtBlue = originalBlue[i][j];
        }
    }
        ...
        for (int j = 0; j < width; j++)
        {
            ...
            for (int k = i - 1; k <= i + 1; k++)
            {
                for (int l = j - 1; l <= j + 1; l++)
                {
                    if (k >= 0 && k < height && l >= 0 && l < width)
                    {
                        xSumRed += originalRed[k][l] * Gx[counter];
                        xSumGreen += originalGreen[k][l] * Gx[counter];
                        xSumBlue += originalBlue[k][l] * Gx[counter];

                        ySumRed += originalRed[k][l] * Gy[counter];
                        ySumGreen += originalGreen[k][l] * Gy[counter];
                        ySumBlue += originalBlue[k][l] * Gy[counter];
            ...
            sumRed = sqrt(pow(xSumRed, 2) + pow(ySumRed, 2));
            sumGreen = sqrt(pow(xSumGreen, 2) + pow(ySumGreen, 2));
            sumBlue = sqrt(pow(xSumBlue, 2) + pow(ySumBlue, 2));
            ...
            image[i][j].rgbtRed = round(sumRed);
            image[i][j].rgbtGreen = round(sumGreen);
            image[i][j].rgbtBlue = round(sumBlue);
        }
    }
}
